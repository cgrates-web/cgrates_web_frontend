...
describe 'Integration: FilterNumber', ->
  ...
  describe 'entering valid number', ->
    it 'sends associated action', ->
      ...
      @render(hbs "{{filter-number label='Test' key='test' value=value onValueChange=(action pushValue) step='0.01'}}")
