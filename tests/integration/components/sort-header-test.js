  ...
  describe('clicking', function () {
    describe('not a current column', () =>
      it('sets column as current with ascending sorting', async function () {
        ...
        await render(hbs("{{sort-header key='testkey' sortColumn='other' sortOrder='desc' sortAction=(action sortBy)}}"));
        await click('a');
        expect(this.get('actionTriggered')).to.be.true;
      })
    );
    ...
    describe('current column, ascending sorting', () =>
      it('leaves column as current and sets descending sorting', async function () {
        ...
        await render(hbs("{{sort-header key='testkey' sortColumn='testkey' sortOrder='asc' sortAction=(action sortBy)}}"));
        await click('a');
        expect(this.get('actionTriggered')).to.be.true;
      })
    );
    ...
    describe('current column, descending sorting', () =>
      it('leaves column as current and sets ascending sorting', async function () {
        ...
        await render(hbs("{{sort-header key='testkey' sortColumn='testkey' sortOrder='desc' sortAction=(action sortBy)}}"));
        await click('a');
        expect(this.get('actionTriggered')).to.be.true;
      })
    );
