...
describe('Acceptance: TpChargers.Edit', function () {
  ...
  beforeEach(async function () {
    ...
    server.create('tp-charger', {
      id: '1', tpid: this.tariffPlan.alias, filterIds: 'filter_id1', attributeIds: 'attr_id1'
    });
    server.create('tp-filter', { tpid: this.tariffPlan.alias, customId: 'filter_id2' });
    server.create('tp-attribute', { tpid: this.tariffPlan.alias, customId: 'attr_id2' });
    ...
    it('sends correct data to the backend', async function () {
      ...
      server.patch('/tp-chargers/:id', function (schema, request) {
        expectCorrectRequest = () => {
          ...
          expect(params.data.attributes['filter-ids']).to.eq('filter_id1,filter_id2');
          expect(params.data.attributes['attribute-ids']).to.eq('attr_id1,attr_id2');
      ...
      await selectSearch('[data-test-select-search-to-str="filter-ids"]', 'filter_id2');
      await selectChoose('[data-test-select-search-to-str="filter-ids"]', 'filter_id2');
      await selectSearch('[data-test-select-search-to-str="attribute-ids"]', 'attr_id2');
      await selectChoose('[data-test-select-search-to-str="attribute-ids"]', 'attr_id2');
