...
describe('Acceptance: TpThresholds.Index', function () {
  ...
  describe('select tp-threshold', () =>
    it('reditects to tp-thresholds page', async function () {
      ...
      expect(currentRouteName()).to.equal('tariff-plan.tp-thresholds.tp-threshold.index');
    })
  );
      ...
      server.get('/tp-thresholds', function (schema, request) {
        ...
        switch (counter) {
          case 1:
            ...
            expect(isBlank(request.queryParams['filter[action_ids]'])).to.eq(true);
            expect(isBlank(request.queryParams['filter[activation_interval]'])).to.eq(true);
            expect(isBlank(request.queryParams['filter[max_hits]'])).to.eq(true);
            expect(isBlank(request.queryParams['filter[min_hits]'])).to.eq(true);
  ...
  describe('click pagination link', () =>
    it('makes a correct pagination query', async function () {
      ...
      server.get('/tp-thresholds', function (schema, request) {
        ...
        return { data: [{id: '1', type: 'tp-threshold'}], meta: {total_pages: 2} };
      });
      await visit('/tariff-plans/1/tp-thresholds');
