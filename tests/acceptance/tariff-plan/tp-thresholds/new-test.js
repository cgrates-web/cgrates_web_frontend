...
describe('Acceptance: TpThresholds.New', function () {
  ...
  beforeEach(async function () {
    ...
    server.create('tp-action', { tpid: this.tariffPlan.alias, tag: 'test_id2' });
  ...
  describe('submit empty form', function () {
    ...
    it('displays action-ids error', function () {
      expect(find('[data-test-select-search-to-str="action-ids"] div')).to.have.class('is-invalid');
      expect(find('[data-test-select-search-to-str="action-ids"] .invalid-feedback')).to.exist;
    });
    ...
    it('displays max-hits error', async function () {
      ...
      expect(find('[data-test-max-hits] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays min-hits error', async function () {
      expect(find('[data-test-min-hits] input')).to.have.class('is-invalid');
      expect(find('[data-test-min-hits] .invalid-feedback')).to.have.class('d-block');
    });
  ...
  describe('fill form with correct data and submit', () =>
    it('sends correct data to the backend', async function () {
      ...
      await selectSearch('[data-test-select-search-to-str="action-ids"]', 'test_id2');
      await selectChoose('[data-test-select-search-to-str="action-ids"]', 'test_id2');
