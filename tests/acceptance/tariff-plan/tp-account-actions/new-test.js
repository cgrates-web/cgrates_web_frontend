...
describe('Acceptance: TpAccountActions.New', function () {
  ...
  beforeEach(async function () {
    ...
    server.create('tp-action-plan', { tpid: this.tariffPlan.alias, tag: 'tag-1' });
    server.create('tp-action-trigger', { tpid: this.tariffPlan.alias, tag: 'tag-1' });
  ...
  describe('submit empty form', function () {
    ...
    it('does not displays actions-tag error', function () {
      expect(find('[data-test-select-search-to-str="action-plan-tag"] div')).not.to.have.class('is-invalid');
      expect(find('[data-test-select-search-to-str="action-plan-tag"] .invalid-feedback')).not.to.exist;
    });
    it('does not displays actions-tag error', function () {
      expect(find('[data-test-select-search-to-str="action-trigger-tag"] div')).not.to.have.class('is-invalid');
      expect(find('[data-test-select-search-to-str="action-trigger-tag"] .invalid-feedback')).not.to.exist;
    });
  });
  ...
  describe('fill form with correct data and submit', () =>
    it('sends correct data to the backend', async function () {
      ...
      await selectSearch('[data-test-select-search-to-str="action-plan-tag"]', 'tag-1');
      await selectChoose('[data-test-select-search-to-str="action-plan-tag"]', 'tag-1');
      await selectSearch('[data-test-select-search-to-str="action-trigger-tag"]', 'tag-1');
      await selectChoose('[data-test-select-search-to-str="action-trigger-tag"]', 'tag-1');
