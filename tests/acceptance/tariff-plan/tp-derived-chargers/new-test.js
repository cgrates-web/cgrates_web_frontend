...
describe('Acceptance: TpDerivedChargers.New', function () {
  ...
  describe('submit empty form', function () {
    ...
    it('displays runid error', async function () {
      ...
      expect(find('[data-test-runid] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays run-filters error', async function () {
      expect(find('[data-test-run-filters] input')).to.have.class('is-invalid');
      expect(find('[data-test-run-filters] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays req-type-field error', async function () {
      expect(find('[data-test-req-type-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-req-type-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays direction-field error', async function () {
      expect(find('[data-test-direction-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-direction-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays tenant-field error', async function () {
      expect(find('[data-test-tenant-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-tenant-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays category-field error', async function () {
      expect(find('[data-test-category-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-category-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays account-field error', async function () {
      expect(find('[data-test-account-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-account-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays subject-field error', async function () {
      expect(find('[data-test-subject-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-subject-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays destination-field error', async function () {
      expect(find('[data-test-destination-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-destination-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays setup-time-field error', async function () {
      expect(find('[data-test-setup-time-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-setup-time-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays pdd-field error', async function () {
      expect(find('[data-test-pdd-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-pdd-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays answer-time-field error', async function () {
      expect(find('[data-test-answer-time-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-answer-time-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays usage-field error', async function () {
      expect(find('[data-test-usage-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-usage-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays supplier-field error', async function () {
      expect(find('[data-test-supplier-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-supplier-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays disconnect-cause-field error', async function () {
      expect(find('[data-test-disconnect-cause-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-disconnect-cause-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays rated-field error', async function () {
      expect(find('[data-test-rated-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-rated-field] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays cost-field error', async function () {
      expect(find('[data-test-cost-field] input')).to.have.class('is-invalid');
      expect(find('[data-test-cost-field] .invalid-feedback')).to.have.class('d-block');
    });
  });
    ...
    it('sends correct data to the backend', async function () {
      ...
      server.post('/tp-derived-chargers', function (schema, request) {
        expectCorrectRequest = () => {
          ...
          expect(params.data.attributes['req-type-field']).to.eq('req-type-field');
          expect(params.data.attributes['direction-field']).to.eq('direction-field');
          expect(params.data.attributes['tenant-field']).to.eq('tenant-field');
          expect(params.data.attributes['category-field']).to.eq('category-field');
          expect(params.data.attributes['account-field']).to.eq('account-field');
          expect(params.data.attributes['subject-field']).to.eq('subject-field');
          expect(params.data.attributes['destination-field']).to.eq('destination-field');
          expect(params.data.attributes['destination-field']).to.eq('destination-field');
          expect(params.data.attributes['setup-time-field']).to.eq('setup-time-field');
          expect(params.data.attributes['pdd-field']).to.eq('pdd-field');
          expect(params.data.attributes['answer-time-field']).to.eq('answer-time-field');
          expect(params.data.attributes['usage-field']).to.eq('usage-field');
          expect(params.data.attributes['supplier-field']).to.eq('supplier-field');
          expect(params.data.attributes['disconnect-cause-field']).to.eq('disconnect-cause-field');
      ...
      await selectSearch('[data-test-select-search-to-str="destination-ids"]', 'tag-1');
      await selectChoose('[data-test-select-search-to-str="destination-ids"]', 'tag-1');
      ...
      await fillIn('[data-test-disconnect-cause-field] input', 'disconnect-cause-field');
