...
describe('Acceptance: TpDerivedChargers.Edit', function () {
  ...
  beforeEach(async function () {
    ...
    server.create('tp-derived-charger', { tpid: this.tariffPlan.alias, destinationIds: 'tag-1' });
    server.create('tp-destination', { id: '1', tpid: this.tariffPlan.alias, tag: 'tag-2' });
    ...
    it('sends correct data to the backend', async function () {
      ...
      server.patch('/tp-derived-chargers/:id', function (schema, request) {
        expectCorrectRequest = () => {
          ...
          expect(params.data.attributes['destination-ids']).to.eq('tag-1,tag-2');
      ...
      await selectSearch('[data-test-select-search-to-str="destination-ids"]', 'tag-2');
      await selectChoose('[data-test-select-search-to-str="destination-ids"]', 'tag-2');
