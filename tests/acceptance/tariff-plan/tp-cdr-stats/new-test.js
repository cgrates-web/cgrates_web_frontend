...
describe('Acceptance: TpCdrStats.New', function () {
  ...
  describe('submit empty form', function () {
    ...
    it('displays queue-length error', async function () {
      expect(find('[data-test-queue-length] input')).to.have.class(
        'is-invalid'
      );
      expect(find('[data-test-queue-length] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    ...
    it('displays time-window error', async function () {
      ...
      expect(
        find('[data-test-input-interval="time-window"] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays save-interval error', async function () {
      expect(
        find('[data-test-input-interval="save-interval"] input')
      ).to.have.class('is-invalid');
      expect(
        find('[data-test-input-interval="save-interval"] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays setup-interval error', async function () {
      expect(find('[data-test-setup-interval] input')).to.have.class(
        'is-invalid'
      );
      expect(
        find('[data-test-setup-interval] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays tors error', async function () {
      expect(find('[data-test-tors] input')).to.have.class('is-invalid');
      expect(find('[data-test-tors] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    it('displays cdr-hosts error', async function () {
      expect(find('[data-test-cdr-hosts] input')).to.have.class('is-invalid');
      expect(find('[data-test-cdr-hosts] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    it('displays cdr-sources error', async function () {
      expect(find('[data-test-cdr-sources] input')).to.have.class('is-invalid');
      expect(find('[data-test-cdr-sources] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    ...
    it('displays tenants error', async function () {
      ...
      expect(find('[data-test-tenants] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    ...
    it('displays accounts error', async function () {
      ...
      expect(find('[data-test-accounts] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    it('displays subjects error', async function () {
      expect(find('[data-test-subjects] input')).to.have.class('is-invalid');
      expect(find('[data-test-subjects] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    ...
    it('displays pdd-interval error', async function () {
      expect(find('[data-test-pdd-interval] input')).to.have.class(
        'is-invalid'
      );
      expect(find('[data-test-pdd-interval] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    it('displays usage-interval error', async function () {
      expect(find('[data-test-usage-interval] input')).to.have.class(
        'is-invalid'
      );
      expect(
        find('[data-test-usage-interval] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays suppliers error', async function () {
      expect(find('[data-test-suppliers] input')).to.have.class('is-invalid');
      expect(find('[data-test-suppliers] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    it('displays disconnect-causes error', async function () {
      expect(find('[data-test-disconnect-causes] input')).to.have.class(
        'is-invalid'
      );
      expect(
        find('[data-test-disconnect-causes] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays mediation-runids error', async function () {
      expect(find('[data-test-mediation-runids] input')).to.have.class(
        'is-invalid'
      );
      expect(
        find('[data-test-mediation-runids] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays rated-accounts error', async function () {
      expect(find('[data-test-rated-accounts] input')).to.have.class(
        'is-invalid'
      );
      expect(
        find('[data-test-rated-accounts] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays rated-subjects error', async function () {
      expect(find('[data-test-rated-subjects] input')).to.have.class(
        'is-invalid'
      );
      expect(
        find('[data-test-rated-subjects] .invalid-feedback')
      ).to.have.class('d-block');
    });
    it('displays cost-interval error', async function () {
      expect(find('[data-test-cost-interval] input')).to.have.class(
        'is-invalid'
      );
      expect(find('[data-test-cost-interval] .invalid-feedback')).to.have.class(
        'd-block'
      );
    });
    it('displays action-triggers error', function () {
      expect(
        find('[data-test-select-search-to-str="action-triggers"] div')
      ).to.have.class('is-invalid');
      expect(
        find(
          '[data-test-select-search-to-str="action-triggers"] .invalid-feedback'
        )
      ).to.exist;
    });
  });
    ...
    it('sends correct data to the backend', async function () {
      ...
      server.post('/tp-cdr-stats', function (schema, request) {
        expectCorrectRequest = () => {
          ...
          expect(params.data.attributes['setup-interval']).to.eq(
            'setup-interval'
          );
          ...
          expect(params.data.attributes['usage-interval']).to.eq(
            'usage-interval'
          );
          expect(params.data.attributes['suppliers']).to.eq('suppliers');
          expect(params.data.attributes['disconnect-causes']).to.eq(
            'disconnect-causes'
          );
          expect(params.data.attributes['mediation-runids']).to.eq(
            'mediation-runids'
          );
          expect(params.data.attributes['rated-accounts']).to.eq(
            'rated-accounts'
          );
          expect(params.data.attributes['rated-subjects']).to.eq(
            'rated-subjects'
          );
          expect(params.data.attributes['cost-interval']).to.eq(
            'cost-interval'
          );
          expect(params.data.attributes['action-triggers']).to.eq(
            'trigger-tag'
          );
        };
      ...
      await selectSearch(
        '[data-test-select-search-to-str="action-triggers"]',
        'trigger-tag'
      );
      await selectChoose(
        '[data-test-select-search-to-str="action-triggers"]',
