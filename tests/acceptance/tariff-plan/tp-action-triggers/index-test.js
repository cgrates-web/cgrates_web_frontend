...
describe('Acceptance: tpActionTriggers.Index', function () {
  ...
  describe('select tp-action-trigger', () =>
    it('reditects to tp-action-trigger page', async function () {
      ...
      expect(currentRouteName()).to.equal(
        'tariff-plan.tp-action-triggers.tp-action-trigger.index'
      );
    }));
      ...
      server.get('/tp-action-triggers', function (schema, request) {
        ...
        switch (counter) {
          case 1:
            ...
            expect(isBlank(request.queryParams['filter[unique_id]'])).to.eq(
              true
            );
            expect(
              isBlank(request.queryParams['filter[threshold_type]'])
            ).to.eq(true);
            expect(
              isBlank(request.queryParams['filter[threshold-value]'])
            ).to.eq(true);
            expect(isBlank(request.queryParams['filter[recurrent]'])).to.eq(
              true
            );
            expect(isBlank(request.queryParams['filter[min_sleep]'])).to.eq(
              true
            );
            expect(isBlank(request.queryParams['filter[expiry_time]'])).to.eq(
              true
            );
            expect(
              isBlank(request.queryParams['filter[activation_time]'])
            ).to.eq(true);
            expect(isBlank(request.queryParams['filter[balance_tag]'])).to.eq(
              true
            );
  ...
  describe('set filters and click download csv button', function () {
    it('sends request to the server with filters', async function () {
      ...
      server.get('/tp-action-triggers/export-to-csv/', function (
        _schema,
        request
      ) {
  ...
  describe('click to upload csv button', function () {
    it('redirects to upload csv page', async function () {
      ...
      expect(currentURL()).to.eq(
        '/tariff-plans/1/tp-action-triggers/csv-import'
      );
    });
  });
  ...
  describe('set filters and click delete all button', function () {
    let expectRequestToBeCorrect = () => expect(false).to.eq(true);
    beforeEach(async function () {
      server.post('/tp-action-triggers/delete-all', function (
        _schema,
        request
      ) {
        expectRequestToBeCorrect = () => {
  ...
  describe('click pagination link', () =>
    it('makes a correct pagination query', async function () {
      ...
      server.get('/tp-action-triggers', function (schema, request) {
        ...
        return {
          data: [{ id: '1', type: 'tp-action-trigger' }],
          meta: { total_pages: 2 },
        };
      });
