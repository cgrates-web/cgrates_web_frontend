...
describe('Acceptance: TpActionTriggers.New', function () {
  ...
  beforeEach(async function () {
    ...
    server.createList('tp-timing', 2, { tpid: this.tariffPlan.alias, tag: 'timing-tag' });
  ...
  describe('submit empty form', function () {
    ...
    it('displays unique-id error', async function () {
      ...
      expect(find('[data-test-unique-id] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays threshold-type error', async function () {
      expect(find('[data-test-select="threshold-type"] div')).to.have.class('is-invalid');
      expect(find('[data-test-select="threshold-type"] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays threshold-value error', async function () {
      expect(find('[data-test-threshold-value] input')).to.have.class('is-invalid');
      expect(find('[data-test-threshold-value] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays min-sleep error', async function () {
      expect(find('[data-test-min-sleep] input')).to.have.class('is-invalid');
      expect(find('[data-test-min-sleep] .invalid-feedback')).to.have.class('d-block');
    });
    ...
    it('displays min-queued-items error', async function () {
      expect(find('[data-test-min-queued-items] input')).to.have.class('is-invalid');
      expect(find('[data-test-min-queued-items] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays actions-tag error', function () {
      expect(find('[data-test-select-search-to-str="actions-tag"] div')).to.have.class('is-invalid');
      expect(find('[data-test-select-search-to-str="actions-tag"] .invalid-feedback')).to.exist;
    });
    ...
    it('displays balance-tag error', async function () {
      expect(find('[data-test-select="balance-tag"] div')).to.have.class('is-invalid');
      expect(find('[data-test-select="balance-tag"] .invalid-feedback')).to.have.class('d-block');
    });
    ...
    it('displays balance-directions error', async function () {
      expect(find('[data-test-select="balance-directions"] div')).to.have.class('is-invalid');
      expect(find('[data-test-select="balance-directions"] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays balance-categories error', async function () {
      expect(find('[data-test-balance-categories] input')).to.have.class('is-invalid');
      expect(find('[data-test-balance-categories] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays balance-shared-groups error', async function () {
      expect(find('[data-test-balance-shared-groups] input')).to.have.class('is-invalid');
      expect(find('[data-test-balance-shared-groups] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays balance-rating-subject error', async function () {
      expect(find('[data-test-balance-rating-subject] input')).to.have.class('is-invalid');
      expect(find('[data-test-balance-rating-subject] .invalid-feedback')).to.have.class('d-block');
    });
    it('displays balance-expiry-time error', async function () {
      expect(find('[data-test-balance-expiry-time] input')).to.have.class('is-invalid');
      expect(find('[data-test-balance-expiry-time] .invalid-feedback')).to.have.class('d-block');
    });
    ...
    it('displays balance-timings-tags error', function () {
      expect(find('[data-test-select-search-to-str="balance-timings-tags"] div')).to.have.class('is-invalid');
      expect(find('[data-test-select-search-to-str="balance-timings-tags"] .invalid-feedback')).to.exist;
    });
    it('displays balance-destination-tags error', function () {
      expect(find('[data-test-select-search-to-str="balance-destination-tags"] div')).to.have.class('is-invalid');
      expect(find('[data-test-select-search-to-str="balance-destination-tags"] .invalid-feedback')).to.exist;
    });
  });
    ...
    it('sends correct data to the backend', async function () {
      ...
      server.post('/tp-action-triggers', function (schema, request) {
        expectCorrectRequest = () => {
          ...
          expect(params.data.attributes['threshold-type']).to.eq('*max_balance');
          ...
          expect(params.data.attributes['activation-time']).to.eq('activation-time');
          ...
          expect(params.data.attributes['balance-categories']).to.eq('balance-categories');
          expect(params.data.attributes['balance-directions']).to.eq('*out');
          expect(params.data.attributes['balance-shared-groups']).to.eq('balance-shared-groups');
          expect(params.data.attributes['balance-rating-subject']).to.eq('balance-rating-subject');
          expect(params.data.attributes['balance-expiry-time']).to.eq('balance-expiry-time');
          expect(params.data.attributes['balance-weight']).to.eq('bw12');
          expect(params.data.attributes['balance-timing-tags']).to.eq('timing-tag');
          expect(params.data.attributes['balance-destination-tags']).to.eq('destination-tag');
      ...
      await selectSearch('[data-test-select-search-to-str="actions-tag"]', 'tag-1');
      await selectChoose('[data-test-select-search-to-str="actions-tag"]', 'tag-1');
      ...
      await fillIn('[data-test-balance-categories] input', 'balance-categories');
      await fillIn('[data-test-balance-shared-groups] input', 'balance-shared-groups');
      await fillIn('[data-test-balance-rating-subject] input', 'balance-rating-subject');
      await fillIn('[data-test-balance-expiry-time] input', 'balance-expiry-time');
      await fillIn('[data-test-balance-weight] input', 'bw12');
      await selectSearch('[data-test-select-search-to-str="balance-timings-tags"]', 'timing-tag');
      await selectChoose('[data-test-select-search-to-str="balance-timings-tags"]', 'timing-tag');
      await selectSearch('[data-test-select-search-to-str="balance-destination-tags"]', 'destination-tag');
      await selectChoose('[data-test-select-search-to-str="balance-destination-tags"]', 'destination-tag');
