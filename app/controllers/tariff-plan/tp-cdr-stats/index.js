...
export default Controller.extend(QueryControllerMixin, {
  ...
  queryParams: ['tag', 'queueLength', 'timeWindow', 'saveInterval', 'metrics', 'setupInterval', 'tors',
    ...
    'ratedAccounts', 'ratedSubjects', 'costInterval', 'actionTriggers', 'sortColumn', 'sortOrder',
    'page', 'pageSize'],

  initEmptyDefaultParams() {
    this.permittedFilters.forEach((item) => {
      if (isBlank(this.get(`${item}`)))
        this.set(item, null);
    });
  },
  ...
  permittedFilters: Object.freeze(['tag', 'queueLength', 'timeWindow', 'saveInterval', 'metrics', 'setupInterval', 'tors',
    'cdrHosts', 'cdrSources', 'reqTypes', 'directions', 'tenants', 'categories', 'accounts', 'subjects',
    'destinationIds', 'pddInterval', 'usageInterval', 'suppliers', 'disconnectCauses', 'mediationRunids',
